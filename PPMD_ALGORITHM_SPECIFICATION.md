...
When a decoded byte equals the escape value, the next decoded byte indicates:
...
RAR archives use the PpmdRAR range coder variant (Section 12.2), not the 7z
variant. The model logic (context tree, updates, SEE, rescaling) is identical
between the two — only the range coder differs.
